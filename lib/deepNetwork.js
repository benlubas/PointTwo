...
class Trainer {
  ...
  inputs; 
  target; 
  constructor(net, inputs, targetOutputs){
    this.net = net; 
    this.inputs = inputs;
    this.outputs = this.targetOutputs; 
  }
  
  //train on one set of input. 
  train(input, target){
    let result = net.go(input); 
    
    //calc total error
    let totalError = 0; 
    for(let i = 0; i < target.length; i++){
      totalError += .5 * Math.pow(target[i] - result[i], 2); 
    }

    //partial deriv of total error with respect to the output i
    let deo; 
    //partial deriv of output n with respect to net input
    let doi; 
    //error w respect to weight n 
    let dew;
    for (let i = 0; i < target.length; i++) {
      deo = result[i] - target[i];
      don = result[i] * (1 - result[1]); 
      for(let j = 0; j < this.net.hidden.length; j++){
